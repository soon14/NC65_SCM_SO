 ...
 * @author 刘志伟
 */
public class ReviseSendStateRule implements ICompareRule<SaleOrderVO> {
  ...
  public void process(SaleOrderVO[] vos, SaleOrderVO[] originVOs) {
    ...
    for (SaleOrderViewVO view : views) {
      ...
      if (openState.isSendOpenForRevise(view, originView, materrialmaps)) {
        openList.add(view);
      }
      else if (closeState.isSendColseForRevise(view, originView, materrialmaps)) {
        closeList.add(view);
      }
    }
    if (openList.size() > 0) {
