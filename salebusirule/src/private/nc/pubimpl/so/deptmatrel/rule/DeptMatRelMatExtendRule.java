...
/**
 * 部门，业务员、物料定义关系：物料属性扩展
 * 
 * @since 6.0
 * @version 2011-4-14 下午03:58:43
 * @author 祝会征
 */
public class DeptMatRelMatExtendRule {
   ...
   * @return
   */
  public List<DeptMatRelParaVO> extendBaseClass(DeptMatRelParaVO[] matchparas) {
    List<DeptMatRelParaVO> paraList = new ArrayList<DeptMatRelParaVO>();
    Set<String> pkSet = new HashSet<String>();
    for (DeptMatRelParaVO para : matchparas) {
      paraList.add(para);
    ...
    for (DeptMatRelParaVO para : matchparas) {
      ...
      if (tempMap.containsKey(pk)) {
        ...
        for (String basecl : temppks) {
          DeptMatRelParaVO basclextend =
              (DeptMatRelParaVO) ObjectUtil.serializableClone(para);
   ...
   * @return
   */
  public List<DeptMatRelParaVO> extendMarSaleClass(DeptMatRelParaVO[] matchparas) {
    List<DeptMatRelParaVO> paraList = new ArrayList<DeptMatRelParaVO>();
    Set<String> pkSet = new HashSet<String>();
    for (DeptMatRelParaVO para : matchparas) {
      paraList.add(para);
