  ...
  private void checkInvSaleorg(TranMatRelVO bill) {
    ...
    try {
      ...
      if (notsaleorgmaterialvids.size() > 0) {
       ...
       /* errmessage.append("]尚未分配到销售组织，不允许定义订单类型物料关系。");*//*多语修改*/
        /* throw new BusinessException(errmessage.toString());*//*多语修改*/
        throw new BusinessException(NCLangResOnserver.getInstance().getStrByID("4006007_0", "04006007-0016", null, new String[]{errmessage.toString()})/*所选物料[{0}]尚未分配到销售组织，不允许定义订单类型物料关系。*/);/*多语修改*/
      }
    }
