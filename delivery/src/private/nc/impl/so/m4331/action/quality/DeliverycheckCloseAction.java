...
import nc.bs.so.m4331.quality.DeliverycheckCloseBP;
 ...
 * @author 祝会征
 */
public class DeliverycheckCloseAction {
  public void closeQualityInfo(DeliveryVO[] bills) {
    TimeLog.logStart();
    TimeLog.info("调用删除BP前执行业务规则"); /*-=notranslate=-*/
    TimeLog.logStart();
    DeliveryCheckVO[] vos = this.getQulityInfos(bills);
    DeliverycheckCloseBP close = new DeliverycheckCloseBP();
    close.close(vos);
  ...
  private DeliveryCheckVO[] getQulityInfos(DeliveryVO[] bills) {
    ...
    sqlwhere.append(" and ");
    sqlwhere.append(DeliveryCheckVO.BOUTENDFLAG, UFBoolean.FALSE);
